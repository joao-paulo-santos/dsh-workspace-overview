...
window.__ModuleLoader__.load({ id: 'dsh-workspace-overview', factory: (require) => {
  ...
  const GithubPill = (props) => {
    ...
    // Upstream dropped `recentWorkspaceId` (dsh 0.1.2): the pill follows the
    // open session's workspace and hides without one.
    const sessionId = props.useSessions((st) => st.current)
    const path = props.useWorkspaces((st) => {
      const hit = sessionId !== undefined
        ? st.items.find((item) => item.sessionIds.includes(sessionId))
        : undefined
      return hit !== undefined ? hit.path : undefined
    })
